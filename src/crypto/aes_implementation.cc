...
void AESImplementation::decrypt(const byte input[], byte output[], const byte key[]) {
    ...
    for(byte i = 0; i < 4; i++){
        this->_round_key[i][0] = expanded_key[this->_b - (16 - i)];
        this->_round_key[i][1] = expanded_key[this->_b - (12 - i)];
        this->_round_key[i][2] = expanded_key[this->_b - (8 - i)];
        this->_round_key[i][3] = expanded_key[this->_b - (4 - i)];
    }
    ...
    for(byte current_round = (this->_number_of_rounds - 1); current_round > 0; current_round--){
        std::cout << "Start of Round " << unsigned(current_round) << std::endl;
        this->outputState("Current State: ");
        for(byte i = 0; i < 4; i++)
            for(byte j = 0; j < 4; j++) {
    ...
    std::cout << "Decryption Process Ended" << std::endl;
}
...
void AESImplementation::KeyExpansion(const byte key[], byte expandedKey[]) const {
    ...
    for(byte rcon = 1; keySizeIterator < this->_b; rcon++){
        ...
        if((this->_key_size == 256) && (keySizeIterator < this->_b)){
            ...
            for(byte i = 0; i < 4; i++){
                expandedKey[keySizeIterator + i] = t[i] ^ expandedKey[keySizeIterator - this->_n + i];
                t[i] ^= expandedKey[keySizeIterator - this->_n - 4 + i];
            }
            //memcpy(expandedKey + (keySizeIterator - 3 - 1), t, 4 * sizeof(byte));
            keySizeIterator += 4;
        }
...
void AESImplementation::InverseMixColumns(byte state[4][4]) {
    this->outputState("Before InvMC: ");
    for(byte i = 0; i < 4; i++){
        byte column[4] = {state[0][i], state[1][i], state[2][i], state[3][i]};

        state[0][i] = galois_mul_14[column[0]] ^ galois_mul_11[column[1]] ^ galois_mul_13[column[2]] ^ galois_mul_9[column[3]];
        state[1][i] = galois_mul_9[column[0]] ^ galois_mul_14[column[1]] ^ galois_mul_11[column[2]] ^ galois_mul_13[column[3]];
...
void AESImplementation::outputRoundKey() {
    ...
    for(byte i = 0; i < 4; i++){
        for(byte j = 0; j < 4; j++)
            std::cout << std::hex << std::setfill('0') << std::setw(2) << unsigned(this->_round_key[j][i]);
    }
    std::cout << std::endl;
#endif
}
